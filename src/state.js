...
function createState() {
  const state = {
    ...
    // Last connect/handshake/socket error message from the active backend,
    // surfaced in the offline banner so "X is offline" can say *why*
    // (connection refused, timeout, etc.) instead of just that it is.
    // Cleared on a successful connect; kept across disconnects otherwise so
    // a client that reconnects mid-outage still sees the reason.
    lastError: null
  };

  function setConnected(connected) {
    state.connectedToBackend = connected;
    if (connected) state.lastError = null;
  }
